...
from .models import BoPhan, ChiTietPhuongAnPhanCongCa, DieuChinhNhanSuCaTruc, DonViToChuc, KipTruc, LichSuLichTruc, LichTrucCa, MauChuKyCaTruc, NgayTrucCa, NhanSu, NhomLichTruc, PhamViNhanSuCaTruc, PhanCongNhanSuHCNgay, PhuongAnPhanCongCa, ThanhVienKipTruc
from .services import actual_shift_staff


class DonViToChucSerializer(serializers.ModelSerializer):
    don_vi_cha_ten = serializers.CharField(source="don_vi_cha.ten_don_vi", read_only=True)
    nha_may_ten = serializers.CharField(source="nha_may.ten_nha_may", read_only=True)
    nha_may_pham_vi = serializers.IntegerField(source="nha_may_pham_vi_id", read_only=True)

    class Meta:
        model = DonViToChuc
        fields = ["id", "ma_don_vi", "ten_don_vi", "loai_don_vi", "don_vi_cha", "don_vi_cha_ten", "nha_may", "nha_may_ten", "nha_may_pham_vi", "thu_tu", "dang_hoat_dong"]

    def validate(self, attrs):
        parent = attrs.get("don_vi_cha", getattr(self.instance, "don_vi_cha", None))
        plant = attrs.get("nha_may", getattr(self.instance, "nha_may", None))
        if parent and plant and parent.nha_may_pham_vi_id and parent.nha_may_pham_vi_id != plant.id:
            raise serializers.ValidationError({"don_vi_cha": "Đơn vị cấp trên phải thuộc cùng nhà máy."})
        return attrs
...
class BoPhanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BoPhan
        fields = ["id", "don_vi", "don_vi_ten", "ma_bo_phan", "ten_bo_phan", "loai_bo_phan", "thu_tu", "dang_hoat_dong"]
...
class NhanSuSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = NhanSu
        fields = ["id", "ma_nhan_vien", "ho_ten", "don_vi", "don_vi_ten", "bo_phan", "bo_phan_ten", "user", "username", "chuc_danh", "dien_thoai", "tu_ngay", "den_ngay", "dang_lam_viec"]
    ...
    def validate(self, attrs):
        ...
        user_plant_id = getattr(getattr(user, "profile", None), "nha_may_id", None) if user else None
        if user and user_plant_id != unit.nha_may_pham_vi_id:
            raise serializers.ValidationError({"user": "Tài khoản phải thuộc cùng nhà máy với nhân sự."})
        return attrs
