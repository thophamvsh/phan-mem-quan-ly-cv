...
def excel_template_h1(request):
    ...
    try:
        ...
        # Header rows
        header_row1 = ["THÔNG SỐ H1"] + [""] * (len(thong_so_to_may) - 1)  # A1:T1
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='Thông số H1', index=False, header=False)

            # Lấy workbook và worksheet để format
            workbook = writer.book
            worksheet = writer.sheets['Thông số H1']

            # Merge cells cho header
            from openpyxl.styles import Alignment, Font, PatternFill, Border, Side

            # Merge A1:T1 cho "THÔNG SỐ H1" (20 cột từ A đến T)
        ...
        response['Content-Disposition'] = 'attachment; filename="ThongSoToMay_Template.xlsx"'
...
def import_excel_h1(request):
    ...
    try:
        ...
        # ===== VALIDATION: Kiểm tra file Excel có phải là file H1 không =====
        # 1. Kiểm tra số hàng và số cột cơ bản
        if len(df) < 27:
            return JsonResponse({
                'error': f'File Excel không hợp lệ: File thông số H1 phải có ít nhất 27 hàng (3 header + 24 data rows). File hiện tại có {len(df)} hàng.',
                'status': 'error',
                'expected_rows': 27,
                'actual_rows': len(df)
            }, status=400)
        ...
        if len(df.columns) < 20:
            return JsonResponse({
                'error': f'File Excel không hợp lệ: File thông số H1 phải có ít nhất 20 cột (A đến T). File hiện tại có {len(df.columns)} cột.',
                'status': 'error',
                'expected_columns': 20,
                'actual_columns': len(df.columns)
            }, status=400)

        # 2. Kiểm tra header row 1 (A1) phải chứa "THÔNG SỐ H1"
        ...
        if "VẬN HÀNH" in str(header_row1).upper():
            return JsonResponse({
                'error': 'File Excel không hợp lệ: Đây là file Thông số Vận hành, không phải file Thông số H1. Vui lòng sử dụng file template Thông số H1.',
                'status': 'error',
                'detected_type': 'thong_so_van_hanh',
                'expected_type': 'thong_so_h1'
            }, status=400)

        # Kiểm tra header có chứa "THÔNG SỐ H1" hoặc "H1"
        if "THÔNG SỐ H1" not in str(header_row1).upper() and "H1" not in str(header_row1).upper():
            return JsonResponse({
                'error': f'File Excel không hợp lệ: Header row 1 (A1) phải chứa "THÔNG SỐ H1". Giá trị hiện tại: "{header_row1}". Vui lòng sử dụng file template Thông số H1.',
                'status': 'error',
                'expected_header': 'THÔNG SỐ H1',
                'actual_header': str(header_row1)
            }, status=400)

        # 3. Kiểm tra header row 2 (A2:T2) phải chứa các tên thông số H1 đúng
        # Định nghĩa danh sách tên thông số H1 bắt buộc (20 thông số)
        expected_params_h1 = [
            "Áp lực nước",
            "Áp lực chèn trục",
            "Lưu lượng chèn trục",
        ...
        missing_params = []
        for idx, expected_param in enumerate(expected_params_h1):
        ...
        if missing_params:
            return JsonResponse({
                'error': f'File Excel không hợp lệ: Header row 2 (tên thông số) không khớp với template Thông số H1. Các cột không hợp lệ:\n' + '\n'.join(missing_params[:5]) + (f'\n... và {len(missing_params) - 5} cột khác' if len(missing_params) > 5 else ''),
                'status': 'error',
        ...
        if data_row_count < 24:
            return JsonResponse({
                'error': f'File Excel không hợp lệ: File thông số H1 phải có đúng 24 hàng dữ liệu (từ A4 đến A27). File hiện tại có {data_row_count} hàng dữ liệu.',
                'status': 'error',
        ...
        if data_row_count >= 48:
            return JsonResponse({
                'error': 'File Excel không hợp lệ: File có quá nhiều hàng dữ liệu (>48). Có thể đây là file Thông số Vận hành (48 slots) thay vì file Thông số H1 (24 slots). Vui lòng sử dụng file template Thông số H1.',
                'status': 'error',
